...
function matchesHouse(ev: EtonEvent, house: typeof HOUSES[0]) {
  ...
  // Word-boundary match for initials to avoid false positives (e.g. "AMM" inside "Hammond")
...
const [submitted, setSubmitted] = useState(false);
  ...
  useEffect(() => {
    if (!showCal || events.length > 0) return;
  ...
  return (
    <div style={{ background: "linear-gradient(180deg, #f6f9fc 0%, #f8fbff 100%)", color: "#0d1829", fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif" }}>
        ...
        <section>
          <div style={{ maxWidth: 1100, margin: "0 auto", padding: "60px 24px 80px" }}>
            ...
            <div style={{ marginBottom: 40 }}>
              ...
              <h2 style={{ fontSize: 36, fontWeight: 700, color: "#1f2937", margin: "0 0 6px", letterSpacing: "-0.5px", lineHeight: 1.1 }}>
                This week
              </h2>
              {eventWeek && (
                <p style={{ fontSize: 16, color: "#6b7280", margin: 0 }}>{eventWeek}</p>
              )}
            </div>
              ...
              {showFilters && (
                <div style={{
                    ...
                    {BLOCKS.map(b => (
                      <label key={b.label} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", padding: "6px 8px", borderRadius: 8, background: selectedBlock?.label === b.label ? "#f0faf4" : "transparent", transition: "background 0.12s" }}>
                        <input
                          type="radio"
                          name="block"
                          checked={selectedBlock?.label === b.label}
                          onChange={() => setSelectedBlock(selectedBlock?.label === b.label ? null : b)}
                          style={{ accentColor: "#2f6b47", width: 15, height: 15, cursor: "pointer" }}
                        />
                    ...
                    {HOUSES.map(h => (
                      <label key={h.short} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", padding: "6px 8px", borderRadius: 8, background: selectedHouse?.short === h.short ? "#eff4ff" : "transparent", transition: "background 0.12s" }}>
                        <input
                          type="radio"
                          name="house"
                          checked={selectedHouse?.short === h.short}
                          onChange={() => setSelectedHouse(selectedHouse?.short === h.short ? null : h)}
                          style={{ accentColor: "#3563e9", width: 15, height: 15, cursor: "pointer", flexShrink: 0 }}
                        />
                  ...
                  <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <button
                      onClick={() => { setSelectedBlock(null); setSelectedHouse(null); }}
                      className="btn-ghost"
                      style={{ background: "none", border: "none", fontSize: 13, fontWeight: 500, color: "#6b7280", cursor: "pointer", padding: "4px 8px", borderRadius: 6 }}
                    >
                      Clear all
                    </button>
                    <button
                      ...
                      className="btn-apply"
                      style={{ background: "#111827", color: "#ffffff", border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
                    >
                      Apply
                    </button>
                  </div>
            ...
            {eventsError && (
              <div style={{ textAlign: "center", padding: "60px 0", color: "#9ca3af", fontSize: 14 }}>
                Could not load calendar. Please try again later.
              </div>
            )}
            ...
            {!eventsLoading && !eventsError && (() => {
              ...
              if (Object.keys(byDate).length === 0) return (
                <p style={{ color: "#9ca3af", fontSize: 14, textAlign: "center", padding: "40px 0" }}>
                  No music, sport, drama, competition or society events this week.
                </p>
              );
                    ...
                    return (
                      <div key={date}>
                        ...
                        <div style={{ marginBottom: 20, paddingBottom: 16, borderBottom: "1px solid #e5e7eb" }}>
                          <span style={{ fontSize: 22, fontWeight: 600, color: "#1f2937" }}>
                            {dayNum} {month}
                          </span>
                          <span style={{ fontSize: 15, fontWeight: 400, color: "#9ca3af", marginLeft: 10 }}>
                            {dayName}
                          </span>
                        </div>
                          ...
                          {evs.map((ev, i) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* Title */}
                                <div
                                  style={{ fontSize: 16, fontWeight: 600, color: "#111827", lineHeight: 1.4, marginTop: 4, textDecoration: cancelled ? "line-through" : "none" }}
                                  dangerouslySetInnerHTML={{ __html: ev.Title.replace(/^Cancelled\s*/i, "") }}
                                />

                                {/* Category pill + sport name */}
                                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginTop: 2 }}>
                                  ...
                                  {cancelled && (
                                    <span style={{ fontSize: 12, fontWeight: 600, color: "#d83c6b", background: "#ffe4ea", padding: "3px 8px", borderRadius: 999 }}>
                                      Cancelled
                                    </span>
                                  )}
                                </div>
