...
export default function Home() {
  ...
  const handleLogout = () => {
    localStorage.removeItem("scholairo_user");
    router.push("/login");
  };
  ...
  return (
    <div style={{ background: "#f6f9fc", color: "#0d1829", fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif" }}>
      ...
      <style>{`
        ...
        .email-card-float { animation: emailFloat 4s ease-in-out 1s infinite; }
        .email-card-float:hover { box-shadow: 0 20px 40px rgba(0,0,0,0.12) !important; }
      `}</style>
      ...
      <nav style={{ background: "rgba(255,255,255,0.85)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", borderBottom: "1px solid #e5e7eb", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div className="nav-links">
            ...
            {userName ? (
              <>
                <span style={{ color: "#4a6280", fontSize: 15, fontWeight: 500 }}>Hi, {userName}</span>
                <button onClick={handleLogout} className="nav-text-link" style={{ background: "none", border: "none", color: "#4a6280", fontSize: 15, fontWeight: 500, cursor: "pointer", fontFamily: "inherit", padding: 0 }}>Log out</button>
              </>
            ) : (
              <a href="/login" className="nav-text-link" style={{ color: "#4a6280", fontSize: 15, textDecoration: "none", fontWeight: 500 }}>Login</a>
            )}
            <a href="#signup" onClick={e => fadeToSection(e as React.MouseEvent<HTMLAnchorElement>, "signup")} className="btn-cta" style={{ background: "#111827", color: "#ffffff", padding: "10px 22px", borderRadius: 8, fontSize: 14, fontWeight: 600, textDecoration: "none", boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}>
              Get Started
            </a>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "120px 24px 120px", position: "relative" }}>
          <div className="hero-grid">
            <motion.div initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.12 } } }}>
              <motion.div variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } }} style={{ display: "inline-block", background: "rgba(110,160,220,0.12)", color: "#4a7cb3", fontSize: 13, fontWeight: 600, padding: "6px 14px", borderRadius: 999, marginBottom: 28, letterSpacing: "0.02em" }}>
                Built exclusively for Eton parents
              </motion.div>
              <motion.h1 variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut" } } }} style={{ fontFamily: "var(--font-playfair), 'Playfair Display', serif", fontSize: "clamp(40px, 4.5vw, 56px)", fontWeight: 700, lineHeight: 1.1, margin: "0 0 24px", letterSpacing: "-0.02em", maxWidth: 520 }}>
              ...
              <motion.p variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } }} style={{ fontSize: 17, lineHeight: 1.75, color: "#4a6280", margin: "0 0 36px", maxWidth: 460 }}>
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } }} style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
                <motion.a href="#signup" onClick={e => fadeToSection(e as React.MouseEvent<HTMLAnchorElement>, "signup")} className="btn-cta" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} style={{ background: "#111827", color: "#ffffff", padding: "14px 26px", borderRadius: 10, fontSize: 15, fontWeight: 600, textDecoration: "none", boxShadow: "0 6px 16px rgba(0,0,0,0.10)", display: "inline-block" }}>
                  Get Started
                </motion.a>
                <motion.a href="#how-it-works" onClick={e => fadeToSection(e as React.MouseEvent<HTMLAnchorElement>, "how-it-works")} className="btn-secondary" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} style={{ background: "#ffffff", color: "#374151", padding: "14px 26px", borderRadius: 10, fontSize: 15, fontWeight: 600, textDecoration: "none", border: "1px solid #e5e7eb", boxShadow: "0 1px 4px rgba(0,0,0,0.04)", display: "inline-block" }}>
                  See How It Works
                </motion.a>
              </motion.div>
            </motion.div>
            ...
            {/* Mock Email Card */}
            <div className="email-card-float" style={{ background: "#ffffff", borderRadius: 16, boxShadow: "0 10px 30px rgba(0,0,0,0.08), 0 2px 8px rgba(0,0,0,0.04)", border: "1px solid #e5e7eb", overflow: "hidden" }} onMouseEnter={() => setEmailHovered(true)} onMouseLeave={() => setEmailHovered(false)}>
              ...
              <div style={{ padding: "24px 24px 28px" }}>
                <div style={{ opacity: emailVisible ? 1 : 0, transition: "opacity 0.5s ease" }}>
                  <div style={{ borderBottom: "1px solid #b8d4ec", paddingBottom: 16, marginBottom: 18 }}>
                    <p style={{ fontSize: 11, color: "#4a6280", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.8px", fontWeight: 600 }}>From Scholairo · {EMAIL_EXAMPLES[emailIndex].date}</p>
                    <h3 style={{ fontFamily: "var(--font-playfair), 'Playfair Display', serif", fontSize: 18, fontWeight: 700, margin: 0, color: "#0d1829" }}>{EMAIL_EXAMPLES[emailIndex].name}&apos;s Weekly Overview</h3>
                  </div>
                  {EMAIL_EXAMPLES[emailIndex].items.map((item) => (
                    <div key={item.subject} style={{ marginBottom: 16 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                        <span style={{ background: "#b8d4ec", color: "#2a4e8a", fontSize: 11, fontWeight: 700, padding: "2px 9px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.4px" }}>{item.subject}</span>
                      </div>
                  ...
                  <div style={{ background: "#f0f6fb", borderRadius: 8, padding: "12px 14px", marginTop: 8, border: "1px solid #b8d4ec" }}>
                    <p style={{ fontSize: 12, color: "#4a6280", margin: 0, fontStyle: "italic" }}>Next Sunday you&apos;ll receive {EMAIL_EXAMPLES[emailIndex].name}&apos;s full weekly overview, including events, news and a summary of the week.</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section style={{ padding: "100px 24px", background: "#ffffff" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, ease: "easeOut" }} style={{ textAlign: "center", marginBottom: 60 }}>
            <h2 style={{ fontFamily: "var(--font-playfair), 'Playfair Display', serif", fontSize: 36, fontWeight: 700, color: "#0d1829", margin: "0 0 16px", letterSpacing: "-0.02em" }}>Why parents love Scholairo</h2>
            <p style={{ fontSize: 18, color: "#6b7280", margin: "0 auto", maxWidth: 600, lineHeight: 1.6 }}>Stay informed about your son&apos;s school life without the noise.</p>
          </motion.div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 28 }}>
            {[
              { icon: "📊", title: "Clear weekly overview", desc: "Get a simple summary of everything happening at Eton this week — from fixtures and activities to important announcements." },
              { icon: "📅", title: "Never miss key events", desc: "Sports fixtures, school productions, exeats, and trips — all in your weekly digest every Sunday morning." },
              { icon: "📰", title: "Live Eton calendar", desc: "Browse what's on at Eton right now — updated live from the school calendar, filtered by year group and house." },
            ].map((card, i) => (
              <motion.div key={card.title} initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: "easeOut", delay: i * 0.1 }} whileHover={{ y: -4, boxShadow: "0 16px 32px rgba(0,0,0,0.08)" }} style={{ background: "#ffffff", borderRadius: 16, padding: 28, border: "1px solid #e5e7eb", boxShadow: "0 6px 18px rgba(0,0,0,0.05)", cursor: "default" }}>
      ...
      <section style={{ background: "#f6f9fc", padding: "0 24px 100px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <motion.a href="/whats-on" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, ease: "easeOut" }} whileHover={{ y: -3, boxShadow: "0 20px 40px rgba(0,0,0,0.09)" }} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 32, background: "#ffffff", borderRadius: 20, padding: "36px 44px", border: "1px solid #e5e7eb", boxShadow: "0 6px 20px rgba(0,0,0,0.06)", textDecoration: "none", flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
              <div style={{ width: 52, height: 52, borderRadius: 14, background: "linear-gradient(135deg, #d4edda, #b8d4ec)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 24 }}>📆</div>
              <div>
                ...
                <h3 style={{ fontFamily: "var(--font-playfair), 'Playfair Display', serif", fontSize: 22, fontWeight: 700, color: "#0d1829", margin: "0 0 6px", letterSpacing: "-0.01em" }}>What&apos;s on at Eton this week</h3>
                <p style={{ fontSize: 15, color: "#6b7280", margin: 0, lineHeight: 1.5 }}>Browse fixtures, concerts, plays, and school events — updated live from the Eton calendar.</p>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#0d1829", color: "#ffffff", borderRadius: 10, padding: "12px 22px", fontSize: 14, fontWeight: 600, flexShrink: 0, whiteSpace: "nowrap" }}>
              View calendar
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ marginLeft: 4 }}><path d="M3 7h8M7.5 3.5L11 7l-3.5 3.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>
          </motion.a>
        </div>
      </section>
      ...
      <section id="how-it-works" style={{ background: "#0d1829", padding: "100px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: "easeOut" }} style={{ textAlign: "center", marginBottom: 72 }}>
            ...
            <h2 style={{ fontFamily: "var(--font-playfair), 'Playfair Display', serif", fontSize: "clamp(28px, 3.5vw, 42px)", fontWeight: 700, margin: "0 auto", maxWidth: 520, color: "#ffffff" }}>How Scholairo works</h2>
          </motion.div>
          <div className="steps-grid">
            {[
              { n: "01", color: "#96C8A2", title: "Set up your son's profile", desc: "Tell us his name, year group, and house at Eton. It takes less than two minutes." },
              { n: "02", color: "#5a9bd6", title: "We pull together what's happening", desc: "Scholairo gathers the week's fixtures, concerts, news, and school activity into a clear, readable summary." },
              { n: "03", color: "#e0a86e", title: "You get a weekly email every Sunday", desc: "A beautifully formatted overview lands in your inbox each Sunday — everything you need to know about his week." },
            ].map((step, i) => (
              <motion.div key={step.n} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: "easeOut", delay: i * 0.12 }} whileHover={{ y: -6 }} style={{ padding: "36px 32px", background: "rgba(255,255,255,0.04)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.08)", position: "relative", borderTop: `3px solid ${step.color}`, cursor: "default" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: step.color, letterSpacing: "1px", marginBottom: 20 }}>{step.n}</div>
                <h3 style={{ fontFamily: "var(--font-playfair), 'Playfair Display', serif", fontSize: 20, fontWeight: 700, margin: "0 0 12px", color: "#ffffff" }}>{step.title}</h3>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div className="stats-grid">
            {[
              ...
              { stat: "Personalised", label: "Tailored to your son's year and house", icon: "✨" },
              { stat: "2 minutes", label: "To set up, then nothing else to do", icon: "⚡" },
            ].map((item, i) => (
              <motion.div key={item.stat} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, ease: "easeOut", delay: i * 0.1 }} whileHover={{ y: -5, boxShadow: "0 10px 24px rgba(0,0,0,0.08)" }} style={{ padding: "28px 24px", background: "#ffffff", borderRadius: 14, border: "1px solid #e5e7eb", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", textAlign: "center", cursor: "default" }}>
      ...
      <section id="pricing" style={{ background: "linear-gradient(180deg, #f6f9fc 0%, #f8fbff 100%)", padding: "100px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, ease: "easeOut" }} style={{ textAlign: "center", marginBottom: 64 }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: "#4a8abf", textTransform: "uppercase", letterSpacing: "2px", marginBottom: 14 }}>Pricing</p>
            <h2 style={{ fontFamily: "var(--font-playfair), 'Playfair Display', serif", fontSize: "clamp(28px, 3.5vw, 42px)", fontWeight: 700, margin: "0 0 16px", color: "#0d1829", letterSpacing: "-0.02em" }}>Simple pricing for parents</h2>
            <p style={{ fontSize: 17, color: "#6b7280", maxWidth: 500, margin: "0 auto", lineHeight: 1.6 }}>Stay on top of your son&apos;s school life with one simple weekly summary.</p>
          </motion.div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 28, maxWidth: 780, margin: "0 auto" }}>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: "easeOut", delay: 0.1 }} whileHover={{ y: -4, boxShadow: "0 20px 40px rgba(0,0,0,0.09)" }} style={{ background: "#ffffff", borderRadius: 16, padding: 36, border: "1px solid #e5e7eb", boxShadow: "0 8px 24px rgba(0,0,0,0.06)", cursor: "default" }}>
              ...
              <p style={{ fontSize: 14, color: "#6b7280", margin: "0 0 24px", lineHeight: 1.5 }}>For parents of one child at Eton.</p>
              <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: 20, marginBottom: 28 }}>
                {["Weekly school summaries", "Eton fixtures & events", "Revision insights", "Parent notifications"].map(f => (
              ...
              <a href="#signup" onClick={e => fadeToSection(e as React.MouseEvent<HTMLAnchorElement>, "signup")} style={{ display: "block", textAlign: "center", background: "#7db78c", color: "#ffffff", padding: "13px 22px", borderRadius: 10, fontWeight: 600, fontSize: 15, textDecoration: "none" }}>Start Free Trial</a>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: "easeOut", delay: 0.2 }} whileHover={{ y: -4, boxShadow: "0 20px 40px rgba(0,0,0,0.09)" }} style={{ background: "#ffffff", borderRadius: 16, padding: 36, border: "2px solid #7db78c", boxShadow: "0 8px 24px rgba(0,0,0,0.06)", position: "relative", cursor: "default" }}>
              <div style={{ position: "absolute", top: -13, left: 32, background: "#7db78c", color: "#ffffff", fontSize: 11, fontWeight: 700, padding: "4px 12px", borderRadius: 999, letterSpacing: "0.4px" }}>Most popular</div>
              ...
              <p style={{ fontSize: 14, color: "#6b7280", margin: "0 0 24px", lineHeight: 1.5 }}>For families with more than one son at Eton.</p>
              <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: 20, marginBottom: 28 }}>
          ...
          <p style={{ textAlign: "center", color: "#9ca3af", fontSize: 14, marginTop: 32 }}>14-day free trial. No credit card required.</p>
        </div>
      </section>
      ...
      <section id="signup" style={{ padding: "80px 24px 100px", background: "linear-gradient(180deg, #f6f9fc 0%, #eef4fb 100%)" }}>
        <div style={{ maxWidth: 580, margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, ease: "easeOut" }} style={{ textAlign: "center", marginBottom: 40 }}>
            ...
            <h2 style={{ fontFamily: "var(--font-playfair), 'Playfair Display', serif", fontSize: "clamp(26px, 3vw, 36px)", fontWeight: 700, color: "#0d1829", margin: "0 0 12px", lineHeight: 1.2 }}>Start receiving weekly summaries</h2>
            <p style={{ fontSize: 16, color: "#6b7280", margin: 0 }}>Free to join. Your first overview arrives this Sunday.</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: "easeOut", delay: 0.1 }} style={{ background: "#ffffff", borderRadius: 20, border: "1px solid #e5e7eb", boxShadow: "0 8px 40px rgba(0,0,0,0.08)", overflow: "hidden" }}>
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit}>
                  <div className="form-row">
                    <div><label style={labelStyle}>Your name</label><input required placeholder="e.g. Sarah Mitchell" style={inputStyle} /></div>
                    <div><label style={labelStyle}>Child&apos;s name</label><input required placeholder="e.g. Ollie" style={inputStyle} /></div>
                  </div>
                  ...
                  <motion.button type="submit" className="btn-submit" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} style={{ width: "100%", background: "#96C8A2", color: "#0d1829", border: "none", borderRadius: 9, padding: "15px 0", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: "0 2px 10px rgba(106,171,120,0.3)" }}>
                    Start my weekly summaries →
                  </motion.button>
                  <p style={{ textAlign: "center", fontSize: 12, color: "#4a6280", marginTop: 14 }}>Free to join. No payment details required.</p>
                </form>
              )}
            </div>
          </motion.div>
...
const labelStyle: React.CSSProperties = { display: "block", fontSize: 13, fontWeight: 600, color: "#0d1829", marginBottom: 6, letterSpacing: "0.1px" };
const inputStyle: React.CSSProperties = { width: "100%", border: "1.5px solid #b8d4ec", borderRadius: 8, padding: "11px 14px", fontSize: 14, color: "#0d1829", background: "#ffffff", outline: "none", fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", appearance: "none" as const };
